...
### Added（新增）
- 新增本地规则分析链路：在未配置云端模型 Key 或模型失败时，后端可基于 RAW/JPG 物理数据生成诊断报告、Lightroom 参数与微调结果。
- 新增风格画像、风格意图解析、风格参数增强与可选联网风格检索，用于更稳定地响应胶片、赛博朋克、清透、人像等风格诉求。
- 新增参考图追色链路：前端支持参考图上传、颜色统计、强度控制与参数融合，并可基于当前融合参数生成 XMP。
- 新增调色助手历史状态、诊断指标面板、前端主流程 hook 与 Playwright smoke test。
- 新增后端 Vitest 测试集与 GitHub Actions CI，覆盖参数 clamp、LLM 配置、本地分析、参考图追色、风格增强与风格检索。

### Changed（变更）
- README 更新为 `V1-test` 同步说明，移除旧的子模块初始化流程，说明 `third_party/emsdk/` 作为本地 WASM 工具链目录处理。
- WASM 构建脚本与 RAW Worker 增强，提升 NEF 解析、预览提取、错误反馈与构建工具链准备的稳定性。
- 右侧面板、中心画布、左侧栏与主页面流程重构，强化诊断、参数、历史、助手对话与下载当前版的联动体验。
- `.gitignore` 增加本地素材与运行产物排除项：`尼康预设/`、`test-image/`、`frontend/test-results/`、`third_party/emsdk/`、`*.before-bootstrap`。

### Fixed（修复）
- 修复未配置模型 Key 时首轮分析或微调链路不可用的问题。
- 修复部分风格请求参数过猛、参数缺少边界控制或前后两次微调变化不清晰的问题。
- 修复参考图追色、XMP 生成与下载当前参数之间的状态衔接问题。

## [0.1.4] — 2026-03-04

### Added（新增）
- 新增“老师批注式”评分展示（总分 + 等级 + 标签），并在诊断报告中按分数动态映射绿→黄→红渐变色。
- 新增严格评分计算逻辑（前端本地计算）：动态范围、曝光分布、色彩基础、噪声风险与可执行性综合打分。

### Changed（变更）
- 中间画布交互升级为商业化手感：滚轮缩放、拖拽平移、边界约束、双击循环缩放（Fit→100%→200%→Fit）。
- 直方图渲染改为按真实 bins 长度自适应铺满，避免 64-bin 数据只显示左半段。
- 右侧报告面板滚动容器修正 `min-h-0` 布局约束，长文本可正常纵向滚动。
- 临时回切到可工作的模型组合（本地运行配置）：`OPENAI_BASE_URL=https://openai.linktre.cc/v1`、`LLM_VISION_MODEL=gemini-3.1-pro-preview`、`LLM_TEXT_MODEL=gpt-5.2`。

### Fixed（修复）
- 后端 Prompt 增加 JSON 稳定输出约束，降低视觉模型返回长文本时的结构不完整概率。
- 后端解析器增强截断 JSON 修复能力（平衡对象提取、尾逗号清理、括号补齐），减少回退默认预设的频率。
- 版本升级到 `v0.1.4`（frontend/backend package version、后端健康检查版本字段、启动 Banner）。
...
### Added（新增）
- 新增 `backend/src/services/reportEnhancer.ts`，对 LLM 原始诊断执行“专家级二次增强”，补齐量化结论、策略 trade-off 与参数级点评。
- `/api/analyze` 增加 `dual_track_validated` 进度事件，显式反馈双轨输入（视觉轨 + 物理轨）已通过校验。

### Changed（变更）
- 版本升级到 `v0.1.2`（frontend/backend package version、后端健康检查版本字段、启动 Banner）。
- System Prompt 增加“报告质量硬约束”，强制输出更细致的专业结论（量化指标引用、策略分段、动作条数下限）。
- NEF 分析请求在前端提交前增加预览图压缩，改善上传耗时与大文件稳定性。
- 诊断报告前端呈现改为更适合长文本阅读（支持换行与更清晰的动作理由展示）。

### Fixed（修复）
- 修复 LLM 返回体结构不稳定导致的前端潜在崩溃：`safeParseAIResponse` 新增字段归一化与容错解析。
- 修复 `/api/refine` 报告深度不一致问题：微调结果也走统一的专家增强链路。
- 修复 413 报错可读性差问题：前端对 `Analyze API 413` 返回明确提示与下一步操作建议。
